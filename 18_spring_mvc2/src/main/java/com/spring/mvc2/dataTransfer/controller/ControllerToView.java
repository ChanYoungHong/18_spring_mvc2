...
public class ControllerToView {
	...
	public ModelAndView modelAndViewEx() {  		// return타입은 String이 아닌 ModelAndView 클래스
		
//		ModelAndView mv = new ModelAndView();		// ModelAndView 객체 생성	
//		mv.setViewName("dataTransfer/memberList");	// setViewName()메서드로 포워딩할 jsp파일 명시
		
		
		// setViewName 메서드 대신 생성자의 인수로 포워딩할 jsp파일 명시
		ModelAndView mv = new ModelAndView("dataTransfer/memberList");		// ModelAndView 객체 생성	
		...
		// .addObject() 메서드로 뷰에서 사용할 데이터를 전송
...
//				message += "</script>";
		...
		String message = "<h2>hahahahahahahahahh</h2>";
	...
	*/
	...
	public ResponseEntity<Object> responseEntityEx() {
		...
		// 2)
		//return new ResponseEntity<Object>("<h1>html 페이지반환 nice to meet you</h1>", HttpStatus.OK);
		
		// 3)
		//return new ResponseEntity<Object>("<h1>html 페이지반환 nice to meet you</h1>", new HttpHeaders(), HttpStatus.OK);
